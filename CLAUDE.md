...
Open plans: `UPDATE_PLAN.md` (in-app self-update, next up) and
`LOCALIZATION_PLAN.md` (after it). `WINDOWS_VM.md` is how to build and test
...
| `Scripts/pkg-scripts/postinstall` | `pkgbuild --scripts` hook: `pkill -x Pomoppi` right after the payload lands, so installing over a running instance doesn't leave the old process fighting the new build for file handles |
