...
| `.github/workflows/windows.yml` | Release-only CI (`workflow_dispatch` + `release: published`, never a per-commit gate) producing x64 release artifacts via `make-windows-app.js --installer` on a `windows-latest` runner — both the `.zip` and the Inno Setup `Pomoppi-Setup-*.exe` (Phase R4); only the Setup `.exe` is attached to a release, the zip stays a workflow artifact. Verified green through several real `workflow_dispatch` runs (R0's toolchain-tag fix, R4's installer flag), though never yet through the actual `release: published` path — see `RELEASING.md` |
