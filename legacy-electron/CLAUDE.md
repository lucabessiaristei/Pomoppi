...
This is the **archived, legacy** Electron implementation — kept for reference
and continuity, no longer the primary build. The primary implementation is
the Swift/AppKit rewrite under `../native/` (see `../native/CLAUDE.md`); build
and ship that one unless you have a specific reason to touch this tree.

`../SPEC.md` (one directory up) is the authoritative *behavior* contract —
if code and spec disagree, the spec wins. Read the section you're touching,
not the whole file. Every bare path below (`main.js`, `lib/`, `renderer/`,
`tools/`, etc.) is relative to this directory, `legacy-electron/`, not the
repo root — all `npm` commands below must be run from inside it.

This file, and everything below, describes only this legacy Electron app.
It has nothing to do with `../native/`'s Swift/AppKit rewrite, which has its
own contract in `../native/CLAUDE.md`.
